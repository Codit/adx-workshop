...
![ADX in a day Badge](/assets/images/lab1/badge.png "ADX in a day Badge")
...
1. Create your free cluster and database here: [Free ADX Cluster](https://aka.ms/kustofree). If you want to open the ADX Web UI in another Tab click on the link holding down the ``CTRL`` Key.
    ...
    ![Create Cluster and Database](/assets/images/lab1/CreateNewCluster.png "Create Cluster and Database")
...
2. Click on the Icon **My Cluster** in the left navigation pane.
    ...
    ![Create free cluster](/assets/images/lab1/free_cluster_create_db.png "Create free cluster")
...
2. To run KQL queries, you must select the **Query** button on the Free Cluster page.
    ...
    ![Query Button on the Free Cluster Page](/assets/images/lab1/free_cluster_query.png "Query Button on the Free Cluster Page")
...
4. Hit the **Run** button. The query will be executed and its result can be seen in the result grid at the bottom of the page.
      ...
      ![Hello World](/assets/images/lab1/hello_world.png "Hello World")
...
1. Go back to the **My Cluster** page, click the **Ingest** button in the tile **Ingest Data**.
      ...
      ![Ingest Data Button](/assets/images/lab1/data_ingest.png "Ingest Data Button")
...
2. Make sure the cluster and the Database fields are correct. In our example the cluster is named ``MyFreeCluster`` and the database is named ``ADX in a day``. Select the option **Existing table**.
      ...
      ![Ingest Data Wizard](/assets/images/lab1/ingest_table.png "Ingest Data Wizard")
...
5. In the list **Schema defining file** select a file. This file is used to determine the schema of the data. One file is autoselected unless you want to change that. In our example it does not matter which file you choose because all files have the same structure, so you can stick with the autoselected file and click **Next: Schema**.
    ...
    ![Ingest Data from storage](/assets/images/lab1/ingest_from_storage.png "Ingest Data from storage")
...
6. Under Data format, make sure you select **Keep current table schema** and deselect **Ignore the first record**. Click on **Next: Start ingestion**.
    ...
    ![Use schema from ingested data](/assets/images/lab1/ingest_from_storage_schema.png "Use schema from ingested data")
  
7. Wait for the ingestion to be completed, and click **Close**.

    ![Ingestion in progress](/assets/images/lab1/ingestion_completed.png "Ingestion in progress")
...
![A typical properties column](/assets/images/lab1/properties_column.png "A typical properties column")
...
![Example result for Question 3.2](/assets/images/lab1/get_schema.png "Example result for Question 3.2")
...
**Example result:**
![Example Result for Question 3.3](/assets/images/lab1/project.png "Example Result for Question 3.3")
...
**Example result:**
  ...
  ![Example result for question 3.5](/assets/images/lab1/top_10_rowCount.png "Example result for question 3.5")
...
| extend rowCount = todecimal(Properties.rowCount) 
| sort by rowCount
| take 1
  ```
</details>
...
**Example result:**
  ...
  ![Example Result for Question 3.6](/assets/images/lab1/rename_reorder.png "Example Result for Question 3.6")
...
| project-reorder Timestamp, fileFormat, rowCount
| distinct fileFormat
  ```
</details>
...
**Example result:**
  ...
  ![Example Result for Question 3.8](/assets/images/lab1/count_by.png "Example Result for Question 3.8")
...
**Example result:**
 ...
 ![Example result for Question 3.9](/assets/images/lab1/pie.png "Example result for Question 3.9")
...
**Example result:**
 ...
 ![Example result for question 3.10](/assets/images/lab1/timeseries.png "Example result for question 3.10")
...
<details>
  ...
  ```kql
logsRaw
| where Timestamp >= datetime(2014-03-08 09:00) and Timestamp <= datetime(2014-03-08 13:00)
...
- You don't have to select a block of code. Based on current cursor position, code that is separated by empty lines is considered a single block of code.
    ...
    ![Two code blocks considered as one](/assets/images/lab1/code_block.png "Two code blocks considered as one")
...
- You can directly insert filters based on data cells selections using `Ctrl`+`Shift`+`Space`
  ...
  ![Insert filters based on data cells](/assets/images/lab1/add_as_filters.gif "Insert filters based on data cells")
...
- **Update Policy** is like an internal ETL. It can help you manipulate or enrich the data as it gets ingested into the source table (e.g. extracting JSON into separate columns, creating a new calculated column, joining the newly ingested records with a static dimension table that is already in your database, etc). For these cases, using an update policy is a very common and powerful practice.
  ...
  ![How an update policy works](/assets/images/lab1/Update_policy.png "How an update policy works")
...
<details>
  ...
  ```kql
.create function with (folder = 'Demo') ManiputatelogsRaw() {
logsRaw
| where Component in (
  'INGESTOR_EXECUTER', 
  'INGESTOR_GATEWAY', 
...
🎉 Congrats! You've completed ADX in a Day Lab 1. Keep going with [**Lab 2: Advanced KQL, Policies and Visualization**](/lab2/README.md)